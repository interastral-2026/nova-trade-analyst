...
async function fetchRealBalances() {
  if (!CB_CONFIG.apiKey || !CB_CONFIG.apiSecret || CB_CONFIG.apiKey.length < 5) return null;
...
async function placeRealOrder(symbol, side, amountEur) {
  ...
  const body = JSON.stringify({
    ...
    order_configuration: { market_market_ioc: { quote_size: amountEur.toFixed(2).toString() } }
  });
  try {
    const response = await axios.post(`${CB_CONFIG.baseUrl}${path}`, body, { headers: getCoinbaseHeaders('POST', path, body) });
    return { success: true, data: response.data, isPaper: false };
  } catch (e) { 
    console.error(`[TRADE_FAIL_${symbol}]:`, e.response?.data || e.message);
...
function getSyntheticAnalysis(symbol, price, candles) {
  ...
  return {
    ...
    reason: `[SYNTHETIC_OVERRIDE] Env variables not detected yet. Running pure technical scan on ${symbol}. Indicators suggest ${isUp ? 'bullish continuation' : 'range consolidation'}.`,
    expectedROI: 3.5,
    isSynthetic: true
  };
}
...
async function getAdvancedAnalysis(symbol, price, candles) {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: [{ parts: [{ text: `ANALYZE ${symbol} at EUR ${price}. 24H DATA: ${JSON.stringify(compactData)}` }] }],
      config: {
        systemInstruction: `YOU ARE A QUANT TRADER. Return ONLY JSON: { "side": "BUY"|"SELL"|"NEUTRAL", "tp": number, "sl": number, "confidence": number, "reason": "string", "expectedROI": number }. Be extremely precise.`,
        temperature: 0.1,
...
function loadState() {
  const defaults = {
    ...
    liquidity: { eur: 15000, usdc: 0 }, dailyStats: { trades: 0, profit: 0 }, diag: "BOOT"
  };
  try {
...
async function syncLiquidity() {
  ...
  if (realBals) {
    ...
    ghostState.diag = "CB_LIVE_LINK";
  } else {
    ghostState.isPaperMode = true;
    ghostState.diag = API_KEY ? "CB_AUTH_REQUIRED" : "ENV_VAR_MISSING";
  }
  saveState();
}
...
async function loop() {
  ...
  try {
    ...
    if (analysis) {
      ...
      if (ghostState.autoPilot && analysis.side === 'BUY' && analysis.confidence >= 75) {
        if (!ghostState.activePositions.some(p => p.symbol === symbol)) {
          const tradeSize = Math.min(500, ghostState.liquidity.eur * 0.1); 
    ...
    ghostState.currentStatus = `SCAN_COMPLETE`;
  } catch (e) { ghostState.currentStatus = "RECOVERY_MODE"; }
  saveState();
}
