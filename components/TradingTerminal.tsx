             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {holdings.length === 0 ? (
                   <div className="col-span-full h-64 flex flex-col items-center justify-center opacity-20 grayscale">
                      ...
                      <p className="uppercase text-[10px] font-black tracking-[0.3em]">No Active Assets to Track</p>
                   </div>
                ) : (
                  holdings.map((pos) => {
                    ...
                    return (
                      <div key={pos.symbol} className={`bg-zinc-900/30 border p-6 rounded-[2.5rem] group transition-all relative overflow-hidden ${pos.isPaper ? 'border-white/5' : 'border-emerald-500/30 shadow-2xl shadow-emerald-500/5'}`}>
                            ...
                            <div>
                              <h4 className="text-lg font-black text-white flex items-center">
                                ...
                                <span className={`text-[7px] ${pos.isPaper ? 'bg-slate-700 text-slate-300' : 'bg-emerald-500 text-black'} px-1.5 py-0.5 ml-3 rounded font-black tracking-widest uppercase`}>
                                  {pos.isPaper ? 'Simulated' : 'Live'}
                                </span>
                              </h4>
                              <p className="text-[9px] text-slate-500 uppercase mt-1 font-black">Spot Price: <span className="text-white">€{pos.currentPrice?.toLocaleString()}</span></p>
                            </div>
                         ...
                         <div className="grid grid-cols-2 gap-y-4 text-[10px] mb-6 border-b border-white/5 pb-6">
                            <div><span className="text-slate-600 uppercase block text-[7px] mb-1 font-black">Avg Entry</span>€{pos.entryPrice?.toLocaleString()}</div>
                            <div><span className="text-emerald-500 uppercase block text-[7px] mb-1 font-black">Exit Target</span>€{pos.tp?.toLocaleString()}</div>
                            <div><span className="text-indigo-400 uppercase block text-[7px] mb-1 font-black">Position Size</span>€{pos.amount?.toFixed(2)}</div>
                            <div><span className="text-rose-500 uppercase block text-[7px] mb-1 font-black">Safety Stop</span>€{pos.sl?.toLocaleString()}</div>
                         </div>
