...
const TradingTerminal: React.FC<TradingTerminalProps> = ({ 
  ...
  const [stats, setStats] = useState({ eur: 0, usdc: 0, trades: 0, profit: 0, isPaper: true, lastSync: '' });
  ...
  return (
    <div className="flex flex-col space-y-6 h-full font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className={`p-5 rounded-[2rem] border transition-all relative overflow-hidden ${stats.isPaper ? 'bg-black border-white/5' : 'bg-[#050508] border-emerald-500/30'}`}>
           {!stats.isPaper && <div className="absolute top-2 right-4 flex items-center space-x-1"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span> <span className="text-[7px] text-emerald-500 font-black uppercase">Live Wallet</span></div>}
           ...
           {stats.lastSync && <p className="text-[6px] text-slate-600 mt-1 uppercase">Synced: {new Date(stats.lastSync).toLocaleTimeString()}</p>}
        </div>
        ...
        <div className="bg-[#050508] border border-emerald-500/20 p-5 rounded-[2rem]">
           <p className="text-[8px] font-black text-emerald-500 uppercase tracking-widest mb-1">TRADES_EXECUTED</p>
        ...
        <div className="bg-[#050508] border border-white/5 p-5 rounded-[2rem]">
           <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">CORE_STATUS</p>
        ...
        <div ref={terminalRef} className="flex-1 overflow-y-auto p-8 custom-scrollbar">
             ...
             <div className="space-y-6">
                {thoughtHistory.length === 0 ? (
                  <div className="h-48 flex flex-col items-center justify-center opacity-20 italic text-xs uppercase tracking-[0.5em]">
                    <i className="fas fa-microchip animate-pulse mb-4 text-2xl"></i>
                    Neural Engine Initializing...
                  </div>
                ) : (
                  thoughtHistory.map((t, i) => (
                    <div key={t.id || i} className={`border-l-2 pl-6 py-2 transition-all relative ${t.confidence >= 75 ? 'border-emerald-500 bg-emerald-500/5' : 'border-white/10'}`}>
           ...
           {activeTab === 'activity' && (
              <div className="space-y-4">
                {lastScans.map((scan) => (
                  <div key={scan.id} className="flex items-center justify-between border-b border-white/5 pb-3">
                    <div className="flex items-center space-x-4">
                      <span className="text-[9px] font-black text-slate-600">[{new Date(scan.timestamp).toLocaleTimeString()}]</span>
                    ...
                    <span className="text-[9px] text-slate-500 italic max-w-sm truncate">{scan.reason}</span>
                  </div>
                ))}
              </div>
           )}
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {holdings.length === 0 ? (
                   <div className="col-span-2 h-48 flex flex-col items-center justify-center opacity-20 grayscale">
                      <p className="text-[10px] font-black uppercase tracking-widest">Awaiting Alpha Trigger...</p>
                   </div>
                ) : (
                  holdings.map((pos) => (
                    <div key={pos.symbol} className="bg-white/[0.02] border border-emerald-500/20 p-5 rounded-2xl relative">
                       <div className="flex justify-between items-center mb-4">
                          <h4 className="text-sm font-black text-white">{pos.symbol}-EUR</h4>
                          <span className="text-[8px] bg-emerald-500 text-black font-black px-2 py-1 rounded">POSITION_OPEN</span>
                       </div>
                       <div className="grid grid-cols-2 gap-4 text-[10px]">
                          <div><span className="text-slate-500 uppercase block text-[7px]">Entry Price</span>€{pos.entryPrice.toLocaleString()}</div>
                          <div><span className="text-emerald-500 uppercase block text-[7px]">Target (TP)</span>€{pos.tp.toLocaleString()}</div>
                          <div><span className="text-indigo-500 uppercase block text-[7px]">Allocated</span>€{pos.amount}</div>
                          <div><span className="text-rose-500 uppercase block text-[7px]">Safety (SL)</span>€{pos.sl.toLocaleString()}</div>
                       </div>
                    </div>
                  ))
                )}
