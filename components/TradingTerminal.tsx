...
const TradingTerminal: React.FC<TradingTerminalProps> = ({ 
  ...
  const fetchState = async () => {
    try {
      ...
      console.error("Failed to fetch terminal state");
    }
  };

  const handlePurge = async () => {
    if (!confirm("Delete all legacy simulation data and logs?")) return;
    try {
      const res = await fetch(`${getApiBase()}/api/ghost/clear-history`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clearPositions: false })
      });
  ...
  return (
    <div className="flex flex-col space-y-6 h-full font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className={`p-5 rounded-3xl border transition-all relative overflow-hidden ${stats.isPaper ? 'bg-zinc-900/50 border-white/5' : 'bg-emerald-950/20 border-emerald-500/30'}`}>
           <p className="text-[8px] font-black text-indigo-400 uppercase tracking-widest mb-1">CASH_LIQUIDITY</p>
        ...
        <div className="bg-zinc-900/50 border border-white/5 p-5 rounded-3xl">
           <p className="text-[8px] font-black text-cyan-400 uppercase tracking-widest mb-1">STABLE_RESERVE</p>
      ...
      <div className="flex-1 bg-[#020205] border border-white/10 rounded-[2rem] overflow-hidden flex flex-col shadow-2xl">
        <div className="px-8 py-5 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
           ...
           {activeTab === 'activity' && (
             <button onClick={handlePurge} className="text-[8px] font-black bg-rose-500/10 border border-rose-500/20 text-rose-500 px-3 py-1 rounded-lg hover:bg-rose-500/20 transition-all uppercase">
               Purge Legacy
             </button>
           )}
        </div>
        ...
        <div ref={terminalRef} className="flex-1 overflow-y-auto p-8 custom-scrollbar">
           {activeTab === 'stream' && (
             <div className="space-y-4">
                {thoughtHistory.length === 0 ? (
                   <div className="h-64 flex items-center justify-center opacity-20 text-[10px] font-black uppercase tracking-widest">No signals detected yet...</div>
                ) : (
                  thoughtHistory.map((t, i) => (
                    <div key={t.id || i} className={`border-l-2 pl-6 py-4 transition-all ${t.side === 'BUY' ? 'border-emerald-500 bg-emerald-500/5' : 'border-white/10'}`}>
                       ...
                       <p className="text-[11px] text-slate-400 leading-relaxed italic">"{t.analysis}"</p>
                    </div>
                  ))
                )}
             </div>
           ...
           {activeTab === 'activity' && (
             <div className="space-y-3">
                {logs.length === 0 ? (
                  <div className="h-48 flex items-center justify-center opacity-20 uppercase text-[10px] font-black">History is clean.</div>
                ) : (
                  logs.map((log) => (
                    <div key={log.id} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl group hover:border-white/10">
                       <div className="flex items-center space-x-4">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] ${log.action === 'BUY' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                             {log.action[0]}
                          </div>
                          <div>
                             <h5 className="text-[11px] font-black text-white">{log.symbol} <span className="text-[8px] text-slate-600 ml-2">{log.details}</span></h5>
           ...
           {activeTab === 'holdings' && (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {holdings.length === 0 ? (
                   <div className="col-span-2 h-48 flex items-center justify-center opacity-20 uppercase text-[10px] font-black">No open trades.</div>
                ) : (
                  holdings.map((pos) => (
                    <div key={pos.symbol} className={`bg-zinc-900/30 border p-6 rounded-3xl group transition-all ${pos.isPaper ? 'border-white/5 opacity-70' : 'border-emerald-500/20'}`}>
                       <div className="flex justify-between items-center mb-6">
                          <div>
                            <h4 className="text-md font-black text-white">{pos.symbol}-EUR</h4>
                            <p className="text-[8px] text-slate-500 uppercase mt-1">Live: €{pos.currentPrice?.toLocaleString()}</p>
                          </div>
                       ...
                       <div className="grid grid-cols-2 gap-y-4 text-[10px] mb-6">
                          <div><span className="text-slate-600 uppercase block text-[7px] mb-1">Entry</span>€{pos.entryPrice?.toLocaleString()}</div>
                          <div><span className="text-emerald-500/70 uppercase block text-[7px] mb-1">TP Target</span>€{pos.tp?.toLocaleString()}</div>
                          <div><span className="text-indigo-400 uppercase block text-[7px] mb-1">Amount</span>€{pos.amount?.toFixed(2)}</div>
                          <div><span className="text-rose-500/70 uppercase block text-[7px] mb-1">Safety SL</span>€{pos.sl?.toLocaleString()}</div>
                       </div>

                       <div className="relative h-1 bg-white/5 rounded-full overflow-hidden">
                          <div 
                            className={`h-full transition-all duration-1000 ${pos.pnlPercent >= 0 ? 'bg-emerald-500' : 'bg-rose-500'}`}
