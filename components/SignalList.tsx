...
interface SignalListProps {
  signals: any[]; // استفاده از any برای انعطاف‌پذیری با مدل‌های مختلف داده بک‌اِند
}

const SignalList: React.FC<SignalListProps> = ({ signals }) => {
  const limitedSignals = Array.isArray(signals) ? signals.slice(0, 15) : [];
              ...
              >
                ...
                <div className="flex justify-between items-center pt-2 border-t border-white/5">
                   <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[8px] font-black text-slate-500">{signal.confidence || 0}%</span>
                   </div>
                   <span className="text-[7px] text-slate-600 font-bold uppercase">{signal.timestamp ? new Date(signal.timestamp).toLocaleTimeString() : 'N/A'}</span>
                </div>
              </div>
            );
          })
